...
def save_metadata(data=None):
    ...
    try:
        ...
        print("DEBUG:    Метаданные успешно сохранены")
...
def load_table_data(table_name: str):
    ...
    try:
        with open(file_path, "r", encoding="utf-8") as f:
            ...
            data = json.loads(content)
            print(
                f"DEBUG:Содержимое после парсинга: {data}, тип: {type(data)}"
            )
            if isinstance(data, str):
                print(
                    f"DEBUG: файл {table_name}.json сщдержит str вместо list"
                )
                if data == table_name:
                    print("DEBUG:В файле сохранено имя таблицы вместо данных")
                    return []
                else:
                    try:
        ...
        print(f"DEBUG:    Ошибка загрузки {table_name}.json")
        return []
...
def save_table_data(table_name: str, data):
    ...
    if not isinstance(data, list):
        print(f"DEBUG:пытаемся сохранить {type(data)} вместо list: {data}")
        return
    try:
        ...
        print(f"DEBUG:Ошибка сохранения {file_path}")
